...
import { Book, Atom, Calculator, ArrowUp, ArrowDown, Moon } from "@phosphor-icons/react";
...
export default function Home() {
  ...
  const uniqueClasses = Array.from(new Set(chaptersData.filter(ch => ch.subject === subject).map(ch => ch.class)));
  const uniqueUnits = Array.from(new Set(chaptersData.filter(ch => ch.subject === subject).map(ch => ch.unit)));

  // Filtering logic
  let filteredChapters = chaptersData.filter((ch) =>
    ch.subject === subject &&
  ...
  filteredChapters = filteredChapters.sort((a, b) => {
    if (sortBy === "chapter") {
      return sortAsc ? a.chapter.localeCompare(b.chapter) : b.chapter.localeCompare(a.chapter);
    } else if (sortBy === "solved") {
      return sortAsc ? (a.questionSolved ?? 0) - (b.questionSolved ?? 0) : (b.questionSolved ?? 0) - (a.questionSolved ?? 0);
    }
    return 0;
  });
    ...
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <aside className="w-64 bg-background border-r flex flex-col py-6 px-4">
        <div className="flex items-center gap-2 mb-8">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">2025 - 2009 | 173 Papers | 15825 Qs</div>
          </div>
        </div>
        <nav className="flex flex-col gap-2">
          {subjects.map(subj => (
            <button
              key={subj}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg ${subject === subj ? 'bg-orange-100 dark:bg-orange-900 text-orange-600 dark:text-orange-300 font-semibold' : 'text-gray-700 dark:text-gray-200 hover:bg-orange-50 dark:hover:bg-gray-800'}`}
              onClick={() => {
                setSubject(subj);
      ...
      <main className="flex-1 px-10 py-8">
        <div className="mb-4 flex flex-col items-center">
          <div className="flex w-full items-center justify-between">
            <div className="text-sm font-semibold text-orange-600 flex items-center justify-center gap-2 w-full text-center">
              ...
              {subject === "Mathematics" && <Calculator size={20} className="text-blue-500" />}
              {subject} PYQs
            </div>
            <button
        ...
        <div className="flex gap-2 mb-2 flex-wrap items-center">
          ...
          >
            <option className="text-foreground bg-background" value="">Class</option>
            {uniqueClasses.map((cls) => (
              <option className="text-foreground bg-background" key={cls} value={cls}>{cls}</option>
            ))}
          </select>
          {/* Unit Filter */}
          <select
          ...
          >
            <option className="text-foreground bg-background" value="">Units</option>
            {uniqueUnits.map((unit) => (
              <option className="text-foreground bg-background" key={unit} value={unit}>{unit}</option>
            ))}
          </select>
          {/* Status Filter */}
          <select
          ...
          >
            <option className="text-foreground bg-background" value="">Status</option>
            {Array.from(new Set(chaptersData.filter(ch => ch.subject === subject).map(ch => ch.status))).map(status => (
              <option className="text-foreground bg-background" key={status} value={status}>{status}</option>
            ))}
          </select>
          ...
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M8 9l4-4 4 4m0 6l-4 4-4-4"/></svg>
            {sortAsc ? 'A-Z' : 'Z-A'}
          </button>
        </div>
        <div className="text-xs text-gray-500 mb-2">Showing all chapters ({filteredChapters.length})</div>
        {/* Chapter List */}
          ...
          {filteredChapters.map((chapter, idx) => (
            <div className="text-foreground bg-background" key={idx} className="flex items-center justify-between bg-white border rounded-lg px-4 py-3 hover:shadow-sm transition cursor-pointer">
              ...
              <div className="flex items-center gap-6 text-xs text-gray-600">
                <span>2025: {chapter.yearWiseQuestionCount["2025"] ?? 0} Qs <ArrowUp size={14} className="inline text-green-500" /></span>
                <span>2024: {chapter.yearWiseQuestionCount["2024"] ?? 0} Qs <ArrowDown size={14} className="inline text-red-500" /></span>
